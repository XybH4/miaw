...
function Auto_Parry.Parry_Data()
    ...
    return {0, directionMap[Auto_Parry.Parry_Type] and directionMap[Auto_Parry.Parry_Type]() or Camera.CFrame, Events, Vector2_Mouse_Location}
end
...
function Auto_Parry.Parry(Parry_Type)
    ...
    if Parry_Method == "Remote" then
        for Remote, Args in pairs(Remotes) do
            Remote:FireServer(nil, Key, Parry_Data[1], Parry_Data[2], Parry_Data[3], Parry_Data[4])
        end
...
local darkPurple = Color3.fromRGB(63, 0, 127)
local darkBlue = Color3.fromRGB(0, 0, 100)
...
RunService.RenderStepped:Connect(function()
    ...
    if primaryPart and ball then
        ...
        local velocityFactor = math.clamp(ball.Velocity.Magnitude / 50, 0, 1)
        visualizer.Color = darkPurple:Lerp(darkBlue, velocityFactor)
    else
        visualizer.Size = Vector3.new(0, 0, 0)
    end
end)
...
ImageButton.Image = "rbxassetid://17547114563"
...
local Dropdown = Tabs.Main:AddDropdown("Dropdown", {
    ...
    Description = "",
    Values = {"Random", "Backwards", "Straight", "Up"},
    Multi = false,
    Default = 3,
...
local Section = Tabs.Main:AddSection("Farm Settings")
